...
function getSetting($key) {
	...
	if ( isset($val) ) {
		...
		if ($key == 'currentstudent' ) $val = validateStudent($val);
...
function validateStudent($currentstudent) {
	global $tbpref;
	if ($currentstudent != '') {
		if (
			get_first_value(
				'select count(StuID) as value from ' . $tbpref . 'students where StuID=' . 
				((int)$currentstudent) 
			) == 0
		)  $currentstudent = ''; 
	}
...
function get_students_selectoptions($v,$dt) {
	global $tbpref;
	$sql = "select StuID, StuName from " . $tbpref . "students order by StuName";
	$res = do_mysqli_query($sql);
	if ( ! isset($v) || trim($v) == '' ) {
		$r = "<option value=\"\" selected=\"selected\">" . $dt . "</option>";
	} else {
	...
	while ($record = mysqli_fetch_assoc($res)) {
		$d = $record["StuName"];
		if ( strlen($d) > 30 ) $d = substr($d,0,30) . "...";
		$r .= "<option value=\"" . $record["StuID"] . "\" " . get_selected($v,$record["StuID"]);
		$r .= ">" . tohtml($d) . "</option>";
	}
	mysqli_free_result($res);
	return $r;
...
function getStudent($sid) {
	...
	$r = get_first_value("select StuName as value from " . $tbpref . "students where StuID='" . $sid . "'");
	if ( isset($r) ) return $r;
	return '';
}
